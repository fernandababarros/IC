...
Nesse estudo analisamos o impacto dos mecanismos de dados ausentes PCA, PA e PNA utilizando o método de Imputação Múltipla, que foi possível realizar a comparação do banco de dados original com os bancos de dados imputados com valores ausentes.

Na seção resultados percebemos como o mecanismo influencia nas inferências e assim vemos que os mecanismos de PCA e PA, por assumirem aleatoriedade, possuem resultados próximos e o mecanismo mais divergente seria o PNA, que não assume aleatoriedade. Vale ressaltar que como a imputação foi feita assumindo a aleatoriedade da PA, percebemos maiores diferenças nos casos de PNA.
