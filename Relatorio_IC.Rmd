...
Pelos box-plots confirmamos a variação maior encontrada na imputação nº 5, quando está é comparada com os dados originais da renda.
...
Pelo gráfico de dispersão acima comprovamos alguns indícios de variações, temos que os dados originais não posseum codificação de dados ausentes para log(renda) ao redor de 5, e as imputações obtiveram valores imputados próximos desse valor, vemos também que a imputação nº 2 a partir de aproximadamente log(renda) igual a 13 não há mais imputações, sendo que nos dados originais percebemos codificação de variável ausente acima desse valor de renda.

Temos abaixo os box-plots para os valores originais e os imputados da renda:
```{r, echo=FALSE, message=FALSE, fig.align='center', out.width = "100%", purl=FALSE}
include_graphics("p61-graf.pdf")
```
