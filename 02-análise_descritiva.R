...
table(Gender)
p2.pryr %<a-% {boxplot(log(Income) ~ Gender, col="grey",
...
p3.pryr %<a-% {boxplot(log(Income) ~ Education, col="grey",
...
table(MarStat)
p4.pryr %<a-% {boxplot(log(Income) ~ MarStat, col="grey",
...
table(Ethnicity)
p5.pryr %<a-% {boxplot(log(Income) ~ Ethnicity, col="grey",
        main="Boxplot da Log(Renda) e a Etnia",
        xlab="Etnia", names=(c("Branco","Negro","Hispânico/Latino","Outros")))
}
...
p6.pryr %<a-% {plot(Age, log(Income), ylab="Log(Renda)", xlab="Idade",
